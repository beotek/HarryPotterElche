...
if (isset($_SESSION['email'])&&$_SESSION['usuario']=='rav'||isset($_SESSION['email'])&&$_SESSION['usuario']=='pre'){
  $usuario=$usr->getBD_usuario($_SESSION['email']);
  $puntosusuario=$usuario['puntos'];
?>

<div>
  <section id=cabecerarav>
  <h1>Sala común Ravenclaw</h1>
  </section>
  <nav class="arbol">Usted está en: <a href="index.php">Inicio</a> / <a href="<?php echo $_SERVER['PHP_SELF'] ?>?p=salarav" <?php if($cssMenu=="salarav"){?>class="current"<?php } ?>>Ravenclaw</a></li> </nav>
<div>
      ...
      <p><a class="btn btn-secondary" href="index.php?p=test6" role="button">Hacer test »</a></p>
        ...
        <div class="col-sm-4 blog-sidebar toppuntos">
          ...
          <div class="sidebar-module sidebar-module-inset jumbotron cambiaravatar">
            ...
            if(isset($_POST['enviar'])){
            	if (is_uploaded_file ($_FILES['img']['tmp_name'])){
                    ...
                    $ruta="./img/avatar/avatarusers/".time().$_FILES['img']['name'];
            				...
            				echo " <form> imagen: ".time().$_FILES['img']['name']. " de ".$tam[0]."x".$tam[1]."px subida correctamente </form>  <a href='index.php?p=salarav'><div class='centrado'><button class='button-naranja' type='button' name='button'>Actualizar</button></a></div";
            ...
            ?>
            	<form action="index.php?p=salarav" method="POST" enctype="multipart/form-data">
            		<p>
